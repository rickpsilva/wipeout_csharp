    ...
    public int[] LoadTexturesFromCmp(string cmpPath)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < images.Length; i++)
            {
                ...
                // ONLY apply this workaround for allsh.cmp which has enough textures
                if (i == 11 && cmpPath.Contains("allsh.cmp", StringComparison.OrdinalIgnoreCase))
                {
                ...
                if (i >= images.Length)
                {
                    _logger?.LogError("Invalid image index {Index} (max={Max})", i, images.Length - 1);
                    continue;
                }
                ...
                {
                    _logger?.LogError("TIM #{Index} returned empty pixels array", i);
                    continue;
                }
                ...
                else
                {
                    _logger?.LogInformation("TIM #{Index}: size={W}x{H}, first pixel RGBA=({R},{G},{B},{A})",
                        i, w, h, pixels.Length > 3 ? pixels[0] : 0, pixels.Length > 3 ? pixels[1] : 0,
                        pixels.Length > 3 ? pixels[2] : 0, pixels.Length > 3 ? pixels[3] : 0);
                }
