...
{
    // 8192
    private const int LZSS_BREAK_EVEN = (1 + LZSS_INDEX_BIT_COUNT + LZSS_LENGTH_BIT_COUNT) / 9;
    ...
    private const int LZSS_WINDOW_SIZE = 1 << LZSS_INDEX_BIT_COUNT;
    ...
    public byte[][] LoadCompressed(string filePath)
    {
        ...
        if (decompressedBytes.Length < totalDecompressedSize)
        {
            _logger.LogWarning("Decompressed data is smaller than expected ({Actual} < {Expected})",
                decompressedBytes.Length, totalDecompressedSize);
        }
        ...
        for (int i = 0; i < imageCount; i++)
        {
            ...
            if (offset + imageSizes[i] > decompressedBytes.Length)
            {
                _logger.LogError("Not enough decompressed data for image {Index}: need {Need} bytes at offset {Offset}, but only have {Have} total",
                    i, imageSizes[i], offset, decompressedBytes.Length);
                images[i] = Array.Empty<byte>();
                continue;
            }
            ...
            offset += imageSizes[i];
            _logger.LogDebug("Split image {Index}: {Size} bytes at offset {Offset}", i, imageSizes[i], offset - imageSizes[i]);
        }
